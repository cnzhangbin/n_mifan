...
Mifan.controller("mDesginCtrl", function($scope, $timeout) {
  ...
  $scope.onSubmit = function() {
    $scope.sendData.content = $scope.mDesignContent;
    $scope.isSending = true;
    $scope.$emit("onMDesignSend", $scope.sendData);
  };
...
Mifan.controller("squareCtrl", function($scope, $http, $random) {
  var MAX_SQUARE_PAGE, setFollowBtn, square, pageSize;
  $scope.$on("$viewContentLoaded", function() {
  ...
  square = {
    ...
    getUsers: function(page) {
      ...
      api = "" + API.squareusers + $scope.privacyParamDir + "/page/" + page + '/pagesize/' + pageSize + "?type=latest";
      if (IsDebug) {
        api = API.squareusers;
      }
    ...
    getUsersCb: function(data) {
      ...
      if (msg === "ok") {
        ...
        $scope.userLists = userLists = result;
        //square.totalUserPage = result["page"]["total_page"];
        square.totalUserPage = MAX_SQUARE_PAGE;
      } else {
        $scope.errorMsg = msg;
      }
